...
@group(4) @binding(0) var tex_sampler: texture_2d<f32>;
@group(5) @binding(0) var<storage, read_write> click_info: array<i32>;
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    if click_info[0] == 1 && selections[id] == 1 { 
        let clicked_color = textureLoad(tex_sampler, vec2(input.ox, input.oy), 0);
        if !(clicked_color.r == 0.0 && clicked_color.g == 0.0 && clicked_color.b == 0.0) {
            positions[id] = positions[id] + vec2(input.x, input.y);
